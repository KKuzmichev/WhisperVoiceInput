...
Push-to-talk голосовой ввод с локальной транскрипцией через [faster-whisper](https://github.com/SYSTRAN/faster-whisper). Распознанный текст печатается в текущее активное окно как будто введён с клавиатуры.
...
- `--output PATH` — путь сохранения расшифровки. Без `--output` `.txt` сохраняется рядом с аудиофайлом с тем же именем. Можно указать файл или директорию (в последнем случае `.txt` с именем аудио сохранится в неё).
